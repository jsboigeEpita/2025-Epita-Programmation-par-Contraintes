...
from heuristics import apply_standard_heuristic, generate_ai_heuristic
...
def solve_advanced_jobshop(jobs_data, maintenance_periods=None, due_dates=None, 
    ...
    Args:
        ...
        heuristic: Optional heuristic name to guide the search ("SPT", "EDD", etc.)
    ...
    if heuristic:
        ...
        apply_standard_heuristic(model, all_tasks, jobs_data, due_dates, heuristic)
...
def solve_with_ai_parameters(jobs_data, maintenance_periods=None, due_dates=None, 
    ...
    # Generate AI heuristic if requested
    heuristic = None
    if use_ai_heuristic:
        print("Generating AI heuristic...")
        heuristic = generate_ai_heuristic(instance)
    
    # Run solver with AI-recommended parameters and heuristic
    ...
    instance, assignments, makespan, tardiness = solve_advanced_jobshop(
        ...
        heuristic=heuristic  # Pass the heuristic to the solver
    )
...
def main():
    ...
    jobs_data = small_instance()  # Vous pouvez changer pour small_instance() ou large_instance()
    ...
    for j1 in range(num_jobs):
        ...
        for j2 in range(num_jobs):
            if j1 != j2:
                setup_times[j1][j2] = {m: 1 for m in range(3)}  # 2 unités de temps pour chaque machine
    ...
    use_ai = True  # Set to False to run without AI
    use_ai_heuristic = True  # Set to False to skip the AI heuristic
    
    if use_ai:
        # Résoudre le problème avec paramètres et heuristique suggérés par l'IA
        ...
        # Résoudre le problème avec des paramètres et heuristique fixes
        ...
        instance, assignments, makespan, tardiness = solve_advanced_jobshop(
            ...
            makespan_weight=1,
            tardiness_weight=1000,
            heuristic="SPT"  # Try different heuristics: "SPT", "EDD", "CR", "MOR"
        )
    ...
    if assignments:
        ...
        visualize_advanced_schedule(instance, assignments, ai_insights=explanation)
